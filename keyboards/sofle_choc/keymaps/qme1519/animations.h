...
#endif
#ifndef ANIM_Y_OFFSET
#    define ANIM_Y_OFFSET 1   // 1 = 8px down, 2 = 16px down
#endif

//-------- CONFIGURATION END--------

#define ANIM_WPM_WIDTH 50
    ...
    if (timer_elapsed32(anim_timer) > ANIM_FRAME_TIME) {
        ...
        for (int offset = 0; offset <= OLED_MATRIX_SIZE; offset++) {
            ...
            if (col < frame_start_offset) {
                ...
                uint8_t row = (offset / OLED_DISPLAY_WIDTH) - ANIM_Y_OFFSET;
                if (row >= ANIM_FRAME_PAGES || (offset / OLED_DISPLAY_WIDTH) < ANIM_Y_OFFSET) {
                    // Outside sprite height → draw background
                    oled_write_raw_byte(ANIM_INVERT ? ~bg : bg, offset);
                    continue;
                }
